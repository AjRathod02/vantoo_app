...
drop policy if exists "Users can insert own history" on public.user_location_history;
create policy "Users can insert own history"
  on public.user_location_history for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can read own history" on public.user_location_history;
