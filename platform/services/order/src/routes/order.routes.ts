...
async function internalAuth(request: import("fastify").FastifyRequest) {
  ...
  if (typeof userId === "string") request.userId = userId;
}

function requireUser(request: import("fastify").FastifyRequest) {
  if (!request.userId) throw AppError.forbidden("User ID required");
...
export async function orderRoutes(app: FastifyInstance) {
  ...
  app.patch("/v1/orders/:id/status", { preHandler: internalAuth }, async (request, reply) => {
    ...
    const order = await orderService.updateStatus(id, status as import("@vantoo/shared").OrderStatus);
    return reply.send({ success: true, data: order });
  });
