...
import type { Order, OrderItem, CreateOrderInput, OrderStatus } from "@vantoo/shared";
...
export class OrderService {
  private async resolveProducts(
    ...
    for (const item of items) {
      const result = await pool.query<ProductSnapshot>(
        `SELECT p.id, p.legacy_id, p.name, p.base_price, p.vendor_id, p.store_id, p.tax_rate,
  ...
  private mapOrder(row: Record<string, unknown>, items: OrderItem[], history?: Order["statusHistory"]): Order {
    return {
      id: row.id as string,
      ...
      address: row.delivery_address as Order["address"],
      deliveryInstructions: row.delivery_instructions as string | undefined,
      estimatedDelivery: row.estimated_delivery ? (row.estimated_delivery as Date).toISOString() : null,
  ...
  async create(userId: string, input: CreateOrderInput): Promise<Order> {
    ...
    if (input.idempotencyKey) {
      const existing = await pool.query(
        `SELECT id FROM orders.orders WHERE metadata->>'idempotencyKey' = $1 AND user_id = $2`,
    ...
    try {
      ...
      if (input.paymentMethod !== "cod") {
        await client.query(
          `INSERT INTO payments.payments (order_id, user_id, amount, method, status, gateway_order_id, gateway_payment_id, idempotency_key, paid_at)
  ...
  async getById(id: string, userId?: string): Promise<Order> {
    ...
    const orderResult = await pool.query(
      `SELECT o.* FROM orders.orders o
       WHERE (o.id::text = $1 OR o.order_number = $1)${userFilter}`,
      values
    );
    ...
    const itemsResult = await pool.query(
      `SELECT product_id, variant_id, product_name, variant_name, sku, image_url,
    ...
    const historyResult = await pool.query(
      `SELECT id, from_status, to_status, note, created_at
       FROM orders.order_status_history WHERE order_id = $1 ORDER BY created_at`,
      [orderResult.rows[0].id]
    );
  ...
  async list(userId: string): Promise<Order[]> {
    ...
    const result = await pool.query(
      `SELECT id FROM orders.orders WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50`,
      [userId]
    );
  ...
  async cancel(id: string, userId: string, reason?: string): Promise<Order> {
    ...
    const pool = getPool();
    await pool.query(
      `UPDATE orders.orders SET status = 'cancelled', cancelled_at = NOW(), cancel_reason = 'customer', cancel_note = $2
       WHERE id = $1`,
      [order.id, reason ?? ""]
    );
    ...
    for (const item of order.items) {
      if (item.variantId) {
        await pool.query(
          `UPDATE catalog.inventory SET quantity = quantity + $1, updated_at = NOW() WHERE variant_id = $2`,
          [item.quantity, item.variantId]
        );
      }
    }
  ...
  async updateStatus(id: string, toStatus: OrderStatus, changedBy?: string): Promise<Order> {
    const pool = getPool();
    const current = await pool.query(`SELECT status, user_id, order_number FROM orders.orders WHERE id = $1`, [id]);
    if (current.rows.length === 0) throw AppError.notFound("Order not found");
    ...
    await pool.query(
      `INSERT INTO orders.order_status_history (order_id, from_status, to_status, changed_by, note)
       VALUES ($1, $2, $3, $4, 'Manual status update')`,
      [id, fromStatus, toStatus, changedBy ?? null]
    );
