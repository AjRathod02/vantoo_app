...
import type { AuthResponse, OtpPurpose } from "@vantoo/shared";
...
export class AuthService {
  ...
  private async findUserByPhone(phone: string): Promise<UserRow | null> {
    const pool = getPool();
    const result = await pool.query<UserRow>(
  ...
  private checkAccountLock(user: UserRow): void {
  ...
  private async registerDevice(userId: string, device?: DeviceInput): Promise<string | null> {
    if (!device?.fingerprint) return null;

    const pool = getPool();
    const result = await pool.query<{ id: string }>(
      `INSERT INTO auth.user_devices (user_id, fingerprint, device_name, platform, push_token)
       VALUES ($1, $2, $3, $4, $5)
  ...
  private async enforceSessionLimit(userId: string): Promise<void> {
    ...
    await pool.query(
      ...
      [userId, env.MAX_SESSIONS_PER_USER - 1]
    );
  }
  ...
  private async createSession(
    ...
    const sessionResult = await pool.query<{ id: string }>(
      `INSERT INTO auth.user_sessions (user_id, device_id, session_token, ip_address, user_agent, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id`,
      [userId, deviceId, sessionToken, ctx.ipAddress ?? null, ctx.userAgent ?? null, expiresAt]
    );
    ...
    await pool.query(
      `INSERT INTO auth.refresh_tokens (user_id, session_id, token_hash, expires_at)
       VALUES ($1, $2, $3, $4)`,
      [userId, sessionId, refreshTokenHash, getRefreshTokenExpiry()]
    );

    await pool.query(
      `UPDATE auth.users SET last_login_at = NOW(), failed_login_attempts = 0, locked_until = NULL WHERE id = $1`,
      [userId]
    );
  ...
  async register(
    ...
    const result = await pool.query<UserRow>(
      `INSERT INTO auth.users (email, phone, password_hash, first_name, last_name, status)
  ...
  async login(
    ...
    if (!valid) {
      ...
      if (attempts >= env.ACCOUNT_LOCKOUT_ATTEMPTS) {
        await pool.query(
          `UPDATE auth.users SET failed_login_attempts = $1, locked_until = NOW() + ($2 || ' minutes')::INTERVAL WHERE id = $3`,
          [attempts, env.ACCOUNT_LOCKOUT_MINUTES, user.id]
        );
  ...
  async verifyOtpLogin(
    ...
    if (!user && (input.purpose === "register" || input.purpose === "phone_verify")) {
      ...
      const result = await pool.query<UserRow>(
        `INSERT INTO auth.users (email, phone, first_name, last_name, status, email_verified, phone_verified)
    ...
    if (!refreshed) throw AppError.internal("User not found after update");
  ...
  async refreshAccessToken(refreshToken: string): Promise<AuthResponse> {
    ...
    const result = await pool.query<{
      ...
      `SELECT rt.id, rt.user_id, rt.session_id, rt.expires_at, rt.is_revoked, s.is_active
    ...
    const sessionResult = await pool.query<{ device_id: string | null }>(
      `SELECT device_id FROM auth.user_sessions WHERE id = $1`,
      [record.session_id]
    );
    ...
    await pool.query(
      `UPDATE auth.refresh_tokens SET is_revoked = TRUE, revoked_at = NOW() WHERE id = $1`,
      [record.id]
    );
    await pool.query(
      `INSERT INTO auth.refresh_tokens (user_id, session_id, token_hash, expires_at)
       VALUES ($1, $2, $3, $4)`,
      [record.user_id, record.session_id, hashToken(newRefreshToken), getRefreshTokenExpiry()]
    );
  ...
  async logout(sessionId: string, userId: string): Promise<void> {
    const pool = getPool();
    await pool.query(
      `UPDATE auth.user_sessions SET is_active = FALSE, revoked_at = NOW() WHERE id = $1 AND user_id = $2`,
      [sessionId, userId]
    );
    await pool.query(
      `UPDATE auth.refresh_tokens SET is_revoked = TRUE, revoked_at = NOW()
  ...
  async resetPassword(email: string, otp: string, newPassword: string): Promise<void> {
    ...
    const history = await pool.query<{ password_hash: string }>(
      `SELECT password_hash FROM auth.password_history WHERE user_id = $1 ORDER BY created_at DESC LIMIT 5`,
      [user.id]
    );
