...
import type { OtpChannel, OtpPurpose } from "@vantoo/shared";
...
export class OtpService {
  async sendOtp(
    ...
    purpose: OtpPurpose,
    ipAddress?: string
  ): Promise<{ expiresIn: number; otpId: string }> {
    const env = loadEnv();
    ...
    const result = await pool.query<OtpRecord>(
      `INSERT INTO auth.otp_verifications (identifier, channel, purpose, otp_hash, max_attempts, expires_at, ip_address)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id`,
      [identifier, channel, purpose, otpHash, env.OTP_MAX_ATTEMPTS, expiresAt, ipAddress ?? null]
    );
  ...
  async verifyOtp(
    ...
    await pool.query(
      `UPDATE auth.otp_verifications SET attempts = attempts + 1 WHERE id = $1`,
      [record.id]
    );
    ...
    await pool.query(
      `UPDATE auth.otp_verifications SET is_verified = TRUE, verified_at = NOW() WHERE id = $1`,
      [record.id]
    );
