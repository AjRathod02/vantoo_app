...
import type { SessionInfo, DeviceInfo } from "@vantoo/shared";
...
export class SessionService {
  async listSessions(userId: string, currentSessionId?: string): Promise<SessionInfo[]> {
    ...
    const result = await pool.query<{
      ...
      `SELECT s.id, d.device_name, COALESCE(d.platform::TEXT, 'unknown') as platform,
    ...
    return result.rows.map((row) => ({
      ...
      platform: row.platform as SessionInfo["platform"],
  ...
  async listDevices(userId: string): Promise<DeviceInfo[]> {
    ...
    const result = await pool.query<{
      ...
      `SELECT id, device_name, platform::TEXT, is_trusted, biometric_enabled, last_seen_at, created_at
       FROM auth.user_devices WHERE user_id = $1 ORDER BY last_seen_at DESC`,
      [userId]
    );
  ...
  async removeDevice(userId: string, deviceId: string): Promise<void> {
    const pool = getPool();
    const result = await pool.query(
      `DELETE FROM auth.user_devices WHERE id = $1 AND user_id = $2 RETURNING id`,
      [deviceId, userId]
    );
  ...
  async validateSession(sessionId: string): Promise<boolean> {
    ...
    const result = await pool.query(
      `SELECT id FROM auth.user_sessions
       WHERE id = $1 AND is_active = TRUE AND expires_at > NOW() AND revoked_at IS NULL`,
      [sessionId]
    );
