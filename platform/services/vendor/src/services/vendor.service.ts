...
import type { Vendor, VendorApplyInput, VendorDashboardStats } from "@vantoo/shared";
import { AppError, slugify } from "../utils/errors.js";

const STORE_TYPE_TO_SERVICE: Record<string, string> = {
  restaurant: "food",
  grocery: "grocery",
  pharmacy: "medicine",
  ecommerce: "ecommerce",
...
const STORE_TYPE_TO_ROLE: Record<string, string> = {
...
export class VendorService {
  async getByUserId(userId: string): Promise<Vendor | null> {
    ...
    const result = await pool.query(
      `SELECT * FROM vendor.vendors WHERE user_id = $1 AND deleted_at IS NULL`,
      [userId]
    );
  ...
  async getById(id: string): Promise<Vendor> {
    const pool = getPool();
    const result = await pool.query(
      `SELECT * FROM vendor.vendors WHERE id = $1 AND deleted_at IS NULL`,
      [id]
    );
    if (result.rows.length === 0) throw AppError.notFound("Vendor not found");
    return mapVendor(result.rows[0]);
  }

  async apply(userId: string, input: VendorApplyInput): Promise<Vendor> {
    ...
    try {
      ...
      const vendorResult = await client.query(
        `INSERT INTO vendor.vendors (user_id, business_name, legal_name, slug, description, contact_email, contact_phone, gst_number, pan_number, status)
      ...
      await client.query(
        `INSERT INTO vendor.stores (vendor_id, name, slug, store_type, service_types, address_line1, city, state, pincode, is_active)
  ...
  async update(userId: string, patch: Record<string, unknown>): Promise<Vendor> {
    ...
    const result = await pool.query(
      `UPDATE vendor.vendors SET ${fields.join(", ")}, updated_at = NOW() WHERE id = $1 RETURNING *`,
      values
    );
    return mapVendor(result.rows[0]);
  }

  async listAll(status?: string): Promise<Vendor[]> {
    const pool = getPool();
  ...
  async approve(vendorId: string, adminUserId: string): Promise<Vendor> {
    ...
    if (result.rows.length === 0) throw AppError.notFound("Vendor not found or already processed");

    await pool.query(
      `UPDATE vendor.stores SET is_active = TRUE, updated_at = NOW() WHERE vendor_id = $1`,
      [vendorId]
    );
  ...
  async reject(vendorId: string, reason: string): Promise<Vendor> {
    const pool = getPool();
    const result = await pool.query(
      `UPDATE vendor.vendors SET status = 'rejected', metadata = jsonb_set(COALESCE(metadata, '{}'), '{rejectionReason}', $2::jsonb), updated_at = NOW()
       WHERE id = $1 RETURNING *`,
      [vendorId, JSON.stringify(reason)]
    );
    if (result.rows.length === 0) throw AppError.notFound("Vendor not found");
  ...
  async suspend(vendorId: string): Promise<Vendor> {
    const pool = getPool();
    const result = await pool.query(
      `UPDATE vendor.vendors SET status = 'suspended', updated_at = NOW() WHERE id = $1 RETURNING *`,
      [vendorId]
    );
    if (result.rows.length === 0) throw AppError.notFound("Vendor not found");
    ...
    const [orders, products, stores] = await Promise.all([
      ...
      pool.query(
        `SELECT COUNT(*)::INT AS total,
         ...
         FROM catalog.products WHERE vendor_id = $1 AND deleted_at IS NULL`,
        [vendorId]
      ),
      pool.query(
        `SELECT COUNT(*)::INT AS count FROM vendor.stores WHERE vendor_id = $1 AND deleted_at IS NULL`,
        [vendorId]
      ),
    ]);
