import { vendorApplySchema, vendorUpdateSchema, documentUploadSchema, storeCreateSchema, storeUpdateSchema, storeTimingSchema, vendorProductSchema, vendorProductUpdateSchema, vendorRejectSchema, vendorOrderStatusSchema, } from "@vantoo/shared";
...
async function requireVendor(request) {
    const userId = requireUser(request);
    const vendor = await vendorService.getByUserId(userId);
    if (!vendor)
        throw AppError.notFound("Vendor profile not found. Please apply first.");
    return vendor;
}
async function requireApprovedVendor(request) {
...
export async function vendorRoutes(app) {
    ...
    app.get("/v1/vendors/orders", { preHandler: internalAuth }, async (request, reply) => {
        const vendor = await requireApprovedVendor(request);
        const pool = getPool();
        const result = await pool.query(`SELECT id FROM orders.orders WHERE vendor_id = $1 ORDER BY created_at DESC LIMIT 50`, [vendor.id]);
        const orders = [];
    ...
    app.patch("/v1/vendors/orders/:orderId/status", { preHandler: internalAuth }, async (request, reply) => {
        ...
        const pool = getPool();
        const current = await pool.query(`SELECT status FROM orders.orders WHERE id = $1 AND vendor_id = $2`, [orderId, vendor.id]);
        ...
        await pool.query(`INSERT INTO orders.order_status_history (order_id, from_status, to_status, note)
       VALUES ($1, $2, $3, 'Vendor status update')`, [orderId, fromStatus, status]);
