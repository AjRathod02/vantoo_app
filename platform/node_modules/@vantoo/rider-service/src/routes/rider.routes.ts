...
import type { OrderStatus } from "@vantoo/shared";

async function requireRider(request: import("fastify").FastifyRequest) {
  const userId = requireUser(request);
  const rider = await riderService.getByUserId(userId);
  if (!rider) throw AppError.notFound("Rider profile not found. Please apply first.");
  return rider;
}

async function requireApprovedRider(request: import("fastify").FastifyRequest) {
...
export async function riderRoutes(app: FastifyInstance) {
  app.get("/v1/riders/me", { preHandler: internalAuth }, async (request, reply) => {
    const userId = requireUser(request);
    const rider = await riderService.getByUserId(userId);
    if (!rider) return reply.send({ success: true, data: null });
  ...
  app.get("/v1/riders/deliveries/available", { preHandler: internalAuth }, async (request, reply) => {
    ...
    return reply.send({ success: true, data: orders.map((o) => ({
      ...
      placedAt: (o.created_at as Date).toISOString(),
    })) });
  });
